...
REVIEWS_HEADERS = ["dish", "name", "rating", "comment"]
...
def load_reviews_for(dish: str):
    """Return a list of dicts: {dish, name, rating:int, comment} for this dish."""
    ensure_reviews_csv()
    rows = []
    ...
    with REVIEWS_CSV.open("r", newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if (row.get("dish") or "").strip().lower() == dish_key:
...
def append_review(dish, name, rating, comment):
    ensure_reviews_csv()
...
@app.route("/meal/i/<int:idx>/review", methods=["GET", "POST"])
def review(dish):
    if request.method == "POST":
        ...
        if not dish or rating == 0:
            abort(400, description="Dish and a 1–5 star rating are required.")

        append_review(dish, name, rating, comment)
        # redirect to clear form and avoid resubmit
        return redirect(url_for("meal_detail_idx", dish=dish))

    # GET request: show page
    reviews = load_reviews_for(dish)
    if reviews:
        avg = round(sum(r["rating"] for r in reviews) / len(reviews), 2)
    else:
        avg = 0
    return render_template("review.html", dish=dish, reviews=reviews, avg=avg)
##########
